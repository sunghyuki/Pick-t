...
router.get("/users/:id/likeLists", user_jwt, async (req, res, next) => {
  try {
    ...
    let likedTrailer = await User.findById(req.params.id).select(
      "-_id -userName -email -password -nickname -phoneNumber -__v -reservationLists"
    );
...
router.delete("/users/:id/likeLists", user_jwt, async (req, res, next) => {
  try {
    ...
    let likedTrailer = await User.findByIdAndUpdate(req.params.id, {
      $pull: { likeLists: { trailerId: req.body.trailerId } },
    });
...
router.put("/users/:id/reservationLists", user_jwt, async (req, res, next) => {
  try {
    ...
    user = await User.findByIdAndUpdate(
      req.params.id,
      { $push: { reservationLists: req.body } },
      {
        new: true,
      }
    );
...
router.get("/users/:id/reservationLists", user_jwt, async (req, res, next) => {
  try {
    ...
    let reservedTrailer = await User.findById(req.params.id).select(
      "-_id -userName -email -password -nickname -phoneNumber -__v -likeLists"
    );
...
router.delete("/users/:id", user_jwt, async (req, res, next) => {
  try {
    ...
    registeredTrailer = await Trailer.findOneAndDelete({
